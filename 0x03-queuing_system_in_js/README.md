...
```
    ...
    - Make sure that the server is working with a ping `src/redis-server ping`
```bash
PONG
```

    - Using the Redis client agian, set the value `School` for the value `ALX`
```bash
